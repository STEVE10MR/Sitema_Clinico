...
public class Enfermero_N implements CRUD<Enfermero>{
    ...
    @Override
    public Enfermero LeerID(int ID_PC) {
        PreparedStatement ps=null;
        ResultSet rs=null;
        Connection con=(Connection)Conexion.Conectar();
        String sql="SELECT E.ID_Enfermero FROM Enfermero E WHERE E.ID_Personal_Clinico=?;";
        try{
            ps=con.prepareStatement(sql);
            ps.setInt(1,ID_PC);
            rs=ps.executeQuery();
    ...
    public void Registrar(Enfermero obj) {
        ...
        ResultSet rs=null;
        Connection con=(Connection)Conexion.Conectar();
        String sql="INSERT INTO Enfermero(ID_Personal_Clinico) VALUES(?);";
        try {
