...
public class Medico_N implements CRUD<Medico>{
    ...
    @Override
    public Medico LeerID(int ID_PC) {
        PreparedStatement ps=null;
        ResultSet rs=null;
        Connection con=(Connection)Conexion.Conectar();
        String sql="SELECT M.ID_Medico FROM Medico M WHERE M.ID_Personal_Clinico=?;";
        try{
            ps=con.prepareStatement(sql);
            ps.setInt(1, ID_PC);
            rs=ps.executeQuery();
    ...
    public void Registrar(Medico obj) {
        ...
        ResultSet rs=null;
        Connection con=(Connection)Conexion.Conectar();
        String sql="INSERT INTO Medico(ID_Personal_Clinico,ID_Especialidad,Limite_Atencion_Med) VALUES(?,?,?);";
        try {
